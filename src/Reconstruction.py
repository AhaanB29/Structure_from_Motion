...
def SfM(seed_pair, all_k, img_id, path):
    ...
    while len(visited_ids) < len(img_id):
        ...
        image_data[best_img_id] = (Rnew, tnew, K_new, ref, desc_new, kp_new)
        ##################################------BA---------######################################
        optimized_points, image_data,ba_success = inter_ba(
        ...
        try:
            ...
            pass
        ##########################################################################
        # Intermediate visualization for debugging
        if(len(visited_ids)%50== 0):
    ...
    final_optimized_points, image_data,final_ba_success = final_ba(
...
if __name__ == '__main__':
    ...
    img_id, descprs, scene_graph,pair,orb = BoW_main(img_path)
    ...
    #pair = ('DSC_0324.JPG', 'DSC_0325.JPG') #Facade ETHZ
    ...
    vis.run()
    vis.destroy_window() 
    Toply(all_pts[:,:3],all_pts[:,3:],filename='temple_new.ply')
